...
下圖（\autoref{fig:input}）為 Tronclass 提供的測試影像。左側為輸入的低解析度影像（64 $\times$ 64），右側為對應的高解析度標準影像（512 $\times$ 512）。
...
\begin{figure}[h]
    \centering
    \subcaptionbox{$K = 2$}{\includegraphics[width=.3\textwidth]{image/normal_K2.png}}
    \hspace{0.25cm}
    \subcaptionbox{$K = 4$}{\includegraphics[width=.3\textwidth]{image/normal_K4.png}}

    \subcaptionbox{$K = 8$}{\includegraphics[width=.3\textwidth]{image/normal_K8.png}}
    \hspace{0.25cm}
    \subcaptionbox{$K = 16$}{\includegraphics[width=.3\textwidth]{image/normal_K16.png}}
    \caption{使用線性正規化的結果}
    \label{fig:normal}
\end{figure}
